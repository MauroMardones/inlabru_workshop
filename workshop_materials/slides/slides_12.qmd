...
- A count process → generates counts, _including_ zeros.
...
The probability of the inflation process $\pi$ is a _hyperparameter_ therefore cannot be modeled with covariates
...
Let's look again at the fishes examples.
We now want to include covariates also in the model for the zero probability:
...
$$
with a _fixed_ and small $\pi$ thus creating a _truncated_ distribution!
