...
- Which models are `inlabru`-friendly?
...
ggdraw() +  draw_plot(p1  +
  annotate("text", x=120, y=10, label="How does the weight of pallid sturgeon vary\n with body length??", size = 6,
              color="red")) +
...
We define **linear predictor** as the mean (or a function of the mean) of our observations **given** the model components.
...
\end{aligned}
$$
**Note**: We assume that, given the  _linear predictor_ $\eta$, the data are independent of each other!

Any dependence in the data is expressed through the _components_ of the linear predictor.
...
$$
**Note:** These always have a Gaussian prior and are used to account for the dependencies among data!
...
digraph posterior {
    ...
    C[label="Bayes Theorem\n &\n Bayesian Computations\n" ]
    D[label="Posterior\n distribution\n" fillcolor = red fontcolor = white]
    E[label="Bayesian Computations are hard!!\n Here is where\n INLA\n comes in!!!\n" fontcolor = red]
...
prior = data.frame(x = seq(-30,20, 0.1)) %>%
  mutate(y = dnorm(x,0,sqrt(1/0.01)))
