---
title: "Lecture 1"
subtitle: Introduction to `inlabru`
author:
  - name: Jafet Belmont 
...
## Some more details I {.smaller auto-animate="true" background-color="#FFFFFF"}
...
## Some more details II {.smaller background-color="#FFFFFF"}
...
:::
 ...
 [**Stage 1** The data generating process]{style="color: #FF6B6B;"}
...
2.  [Linear predictor $\eta_i = \beta_0 + \beta_1 x_i$]{style="color: #FF6B6B;"}

3.  [Latent Gaussian field $\pi(\mathbf{u}|\theta)$]{style="color: #FF6B6B;"}
...
[**Stage 2** The dependence structure]{style="color: #FF6B6B;"}
...
4.  [The hyperparameters: $\pi(\theta)$]{style="color: #FF6B6B;"}
:::
...
[**Stage 3** The hyperparameters]{style="color: #FF6B6B;"}
...
## The posterior distribution {.small background-color="#FFFFFF"}
...
p_cov = fit$marginals.fixed$beta1 %>% ggplot() + geom_line(aes(x,y)) +
  geom_line(data = prior, aes(x,y), linetype= "dashed") + xlab("") + ylab("") +
  ggtitle("Covariate") + coord_cartesian(xlim = c(0.1,0.4))
...
## Real datasets are more complicated! {.smaller}
...
-   Need computationally efficient methods to calculate posteriors (this
    is where INLA helps!).
-   Select priors in a sensible way (we'll talk about this)

## The good news!! {.smaller auto-animate="true"}

In many cases complicated spatio-temporal models are *just* special
cases of the same model structure!! :smiley:

-   **Stage 1**: What is the distribution of the responses?

-   **Stage 2**: What are the model components? and what is their
    distribution?
...
-   **Stage 1**: What is the distribution of the responses?
    ...
    -   We assume data to be *conditionally* independent given the model
        components and some hyperparameters
...
No matter how complicated is your model, the `inlabru` workflow is
*always* the same :smiley:
...
One example with time series: Rainfall over 1 mm in the Tokyo area for
each calendar day during two years (1983-84) are registered.
...
-   $f(\text{time}_t)$ is a RW2 model (this is just a smoother). The
    smoothness is controlled by a hyperparameter $\tau_f$
...
## `inlabru` for time series {auto-animate="true" .smaller }
...
## Bayesian disease mapping {.smaller}
...
> -   **Stage 2:** $\eta_i$ is a linear function of three components: an
>     intercept, a covariate $c_i$, a spatially structured effect
>     $\omega$ likelihood by $$
...
The latent field is
$\boldsymbol{u} = (\beta_0, \beta_1, \omega_1, \omega_2,\ldots, \omega_n)$,
the hyperparameters are $\boldsymbol{\theta} = (\tau_{\omega})$, and
must be given a prior.
...
Encounter probability of Pacific Cod (*Gadus macrocephalus*) from a
trawl survey.
...
-   **Stage 3** Hyperparameters
    ...
    -   Range and sd in the Gaussian field
        $\sigma_{\omega}, \tau_{\omega}$
...
ggplot() + 
  ...
  geom_sf(data = df, pch = ".") +
  xlab("") + ylab("") + ggtitle("Predicted probability")+plot_layout(ncol=2)
...
::: {style="height: 50px;"}
:::
-   Many of the model you have used (and some you have never used but
    will learn about) are *just* special cases of the large class of
    Latent Gaussian models

-   `inlabru` provides an efficient and unified way to fit all these
    models!
